...
//  SimpleMsg
//
//  Created by james luo on 2/12/20.
...
class MessageTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = self.msgAry[indexPath.row]
        // Configure the cell...
    ...
    func readChild(){
        ...
        let msgRef = myRef.child("messages")
        msgRef.observe(.childAdded, with: {(snapshot) in
            print(snapshot)
            self.msgAry.append(snapshot.value as! String)
